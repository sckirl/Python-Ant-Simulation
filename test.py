...
class simulation(object):
    ...
    def leader(self, antID):
        if len(self.path[antID]) > 1:
            self.ants[antID] = antClass(self.ants[antID], self.path[antID].pop())
    ...
    def returner(self, antID): # state of ant after getting food; going to nest
        if self.ants[antID].parent is not None:
            ...
            self.path[antID].append(self.ants[antID].position)
    ...
    def draw(self):
        ...
        for ID in self.lead: # drawing all of the ants
            pygame.draw.circle(win, [10, 255, 0], (self.ants[ID].position[0], self.ants[ID].position[1]), 2)
